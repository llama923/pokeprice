// app.js — Visual card verification with Gemini top pick shown immediately
...
const App = (() => {
  ...
  function setupUpload() {
    ...
    uploadZone.addEventListener('drop',e=>{e.preventDefault();uploadZone.classList.remove('drag-over');addFiles(Array.from(e.dataTransfer.files).filter(f=>f.type.startsWith('image/')));});
    ...
    for(let i=0;i<files.length;i++){
      ...
      try{
        ...
        for(const c of cards){
          ...
          if(c.x!=null&&c.width){try{cropDataUrl=await Gemini.cropCard(file,c);}catch{}}
          identifiedCards.push({...c,rowId:crypto.randomUUID(),condition:'NM',sourceFile:file,cropDataUrl,pickedCard:null,allMatches:null,loaded:false});
        }
      }catch(err){log(`  → Error: ${err.message}`,'error');toast(`Image ${i+1} failed: ${err.message}`,'err');}
    }
  ...
  function buildRowHTML(card) {
    ...
    const cropHtml=card.cropDataUrl
      ?`<img src="${esc(card.cropDataUrl)}" alt="Your card" class="verify-crop-img" />`
      :`<div class="verify-crop-placeholder">No crop</div>`;

    let tcgHtml;
    if(!card.loaded){
      tcgHtml=`<div class="verify-tcg-card"><div class="verify-tcg-img-wrap"><div class="verify-tcg-placeholder"><span class="loading-dots">Loading</span></div></div><div class="verify-tcg-info"><div class="verify-tcg-name">${esc(card.name)}</div><div class="verify-tcg-set" style="color:var(--text-faint)">Searching TCGPlayer...</div></div></div>`;
    } else if(p){
      tcgHtml=`<div class="verify-tcg-card clickable" data-action="pick" title="Click to change version"><div class="verify-tcg-img-wrap"><img src="${esc(p.image)}" alt="${esc(p.name)}" class="verify-tcg-img" onerror="this.style.display='none'" /><div class="verify-tcg-placeholder" style="display:none">No image</div></div><div class="verify-tcg-info"><div class="verify-tcg-name">${esc(p.name)}</div><div class="verify-tcg-set">${esc(p.setName)} ${esc(p.number)}</div>${p.rarity?`<div class="verify-tcg-rarity">${esc(p.rarity)}</div>`:''}</div><div class="verify-change-hint">click to change</div></div>`;
    } else {
      tcgHtml=`<div class="verify-tcg-card clickable not-found" data-action="pick" title="Click to search"><div class="verify-tcg-img-wrap"><div class="verify-tcg-placeholder">?</div></div><div class="verify-tcg-info"><div class="verify-tcg-name">${esc(card.name)}</div><div class="verify-tcg-set" style="color:var(--text-faint)">Not found — click to search</div></div></div>`;
    }

    return`<div class="verify-left"><div class="verify-crop-wrap">${cropHtml}</div><div class="verify-label">Your photo</div></div><div class="verify-arrow">→</div><div class="verify-right">${tcgHtml}</div><div class="verify-controls"><select class="card-condition-sel" data-rowid="${card.rowId}">${['NM','LP','MP','HP','DMG'].map(c=>`<option value="${c}"${c===card.condition?' selected':''}>${c}</option>`).join('')}</select><button class="btn-row-delete" data-rowid="${card.rowId}" title="Remove">✕</button></div>`;
  }

  function bindRowEvents(row,card) {
    row.querySelector('.card-condition-sel')?.addEventListener('change',e=>{
      const c=identifiedCards.find(c=>c.rowId===e.target.dataset.rowid); if(c)c.condition=e.target.value;
    });
    ...
    row.querySelector('.btn-row-delete')?.addEventListener('click',()=>{
      identifiedCards=identifiedCards.filter(c=>c.rowId!==card.rowId); row.remove();
      cardCount.textContent=`${identifiedCards.length} card${identifiedCards.length!==1?'s':''} identified`;
    });
  }
  ...
  async function openPicker(rowId) {
    ...
    if(card.allMatches?.length){renderPickerCards(card.allMatches,grid);}
    else{
      ...
      }catch(err){grid.innerHTML=`<div class="picker-loading" style="color:var(--error)">${esc(err.message)}</div>`;}
    }
    let t;
    $('pickerSearch').oninput=()=>{
      ...
      t=setTimeout(async()=>{
        ...
        try{const r=await TCGDB.searchCard(q);renderPickerCards(r.map(TCGDB.formatCard),grid);}
        catch(err){grid.innerHTML=`<div class="picker-loading" style="color:var(--error)">${esc(err.message)}</div>`;}
      },400);
    };
  }
  ...
  function renderPickerCards(cards,grid) {
    ...
    cards.forEach(card=>{
      ...
      item.innerHTML=`<img src="${esc(card.image)}" alt="${esc(card.name)}" loading="lazy" onerror="this.style.background='var(--bg3)';this.style.minHeight='88px'" /><div class="picker-card-label">${esc(card.setName)}</div><div class="picker-card-number">${esc(card.number)}</div>${card.rarity?`<div class="picker-card-rarity">${esc(card.rarity)}</div>`:''}`;
  ...
  function setupAddCard() {
    btnAddCard.addEventListener('click',()=>{$('manualCardName').value='';$('manualCardSet').value='';$('manualCondition').value='NM';addCardModal.classList.add('open');});
    ...
    btnConfirmAdd.addEventListener('click',()=>{
      const name=$('manualCardName').value.trim(); if(!name){toast('Enter a card name','err');return;}
      const card={name,art_style:'',search_hint:name,alternatives:[],condition:$('manualCondition').value,rowId:crypto.randomUUID(),sourceFile:null,cropDataUrl:null,pickedCard:null,allMatches:null,loaded:false};
      identifiedCards.push(card);
      const row=document.createElement('div'); row.className='verify-row'; row.dataset.rowid=card.rowId;
      row.innerHTML=buildRowHTML(card); bindRowEvents(row,card); cardsList.appendChild(row);
  ...
  async function fetchPrices() {
    ...
    const cardsToPrice=identifiedCards.map(c=>({name:c.pickedCard?c.pickedCard.name:c.name,set:c.pickedCard?`${c.pickedCard.setName} ${c.pickedCard.number}`:'',condition:c.condition}));
  ...
  function showResultsStep() {
    ...
    resultsSummary.innerHTML=`<div class="summary-stat"><div class="stat-value">${total}</div><div class="stat-label">Total Cards</div></div><div class="summary-stat"><div class="stat-value">${priced.length}</div><div class="stat-label">Cards Priced</div></div><div class="summary-stat"><div class="stat-value">$${totalValue.toFixed(2)}</div><div class="stat-label">Collection Value</div></div>`;
  ...
  async function exportToSheets() {
    ...
    try{
      ...
      log(`Exported ${rowsWritten} rows to ${updatedRange}`,'ok'); toast('Exported to Google Sheets! ✓','ok');
  ...
  function exportCSV() {
    ...
    a.click(); URL.revokeObjectURL(url); toast('CSV downloaded!','ok');
  }
